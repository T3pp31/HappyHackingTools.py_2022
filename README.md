...
CTFの問題ファイルや怪しい文字列を安全に解析し、変換・仮説整理・flag候補抽出・writeup作成まで支援するローカルファーストなデスクトップワークベンチです。

A local-first CTF Solver Workbench built with Rust + Tauri v2 and React/TypeScript frontend, designed to help CTF players analyze artifacts, try transformations, collect flag candidates, and export writeups.
...
CTF学習・許可済み検証を目的として作成しています。第三者に対する攻撃に使用しないようにしてください。
本レポジトリ所有者はいかなる責任も負いません。

This tool is designed for CTF learning and authorized testing only, and must not be used for attacks against third parties.
...
| 機能 | 説明 |
|------|------|
| CTF Workspace | 文字列入力、自動検出、Base64/Hex/URL/ROT/Caesar/XOR/JWT 変換、操作履歴、writeup Markdown 生成 |
| CTF Artifact Analyzer | 問題ファイルの magic bytes、file type 推定、hash、entropy、printable strings、flag候補、hex dump 表示 |
| Flag Candidate Detection | `flag{...}` / `ctf{...}` / `picoCTF{...}` / `HTB{...}` など設定可能なflag形式の候補抽出 |
| Web Check | 許可済みWeb問題向けのヘッダー・Cookie・CORS等の確認 |
| Advanced: LAN スキャン | 許可済みNetwork Lab向け。ARP を使用したローカルネットワーク内のホスト検出、ベンダー情報の取得 |
| Advanced: ポートスキャン | 許可済みNetwork Lab向け。指定ホストのオープンポート検出 |
| Advanced: ARP スプーフィング | 許可済みNetwork Lab向け。ARP テーブルのポイズニングとパケットスニッフィング |
...
│   │   ├── Layout/
│   │   └── common/
│   ├── features/                  # CTF変換・検出・writeup生成などの機能モジュール
│   │   └── ctf/
│   ├── pages/                    # ページコンポーネント
...
| `[binary_analysis]` | `max_read_bytes` | ファイル解析で読み込む最大バイト数 |
| `[binary_analysis]` | `hex_preview_bytes` | hex dump 表示対象の最大バイト数 |
| `[binary_analysis]` | `decoded_text_preview_bytes` | decoded text preview の最大バイト数 |
| `[binary_analysis]` | `max_strings` | printable strings の最大表示件数 |
| `[binary_analysis]` | `min_string_length` | printable strings として扱う最小長 |
...
4. CTF Artifact Analyzerで「Select File」を実行し、ファイル選択ダイアログが開くことと、選択ファイルの読み取りが成功することを確認する。
