#[cfg(all(feature = "npcap", target_os = "windows"))]
mod npcap_link {
    /// Npcap SDK の環境変数名
    ...
    /// Npcap SDK の `wpcap.lib` が含まれるディレクトリを検出し、
    /// `cargo:rustc-link-search=native=` で出力する。
    pub fn configure_npcap_link_search() {
        // 環境変数が変更されたら再実行
    ...
    /// wpcap.dll を遅延ロードに設定し、未インストール時もアプリが起動できるようにする
    pub fn configure_delay_load() {
...
fn main() {
    #[cfg(all(feature = "npcap", target_os = "windows"))]
    {
        npcap_link::configure_npcap_link_search();
        npcap_link::configure_delay_load();
    }
    tauri_build::build();
}
